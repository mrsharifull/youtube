    ...
    <div class="x_panel">
      <div class="x_title">
        ...
        <ul class="nav navbar-right panel_toolbox">
            <a href="{{route('video.playlist.create')}}" class="btn btn-outline-info btn-sm">Add Playlist</a>
              ...
              <tbody>
                @foreach ($playlists as $playlist)
                    <tr>
                        <td>{{$playlist->name}}</td>
                        <td>
                            <span class="badge {{$playlist->status == 1 ? 'badge-success' : 'badge-warning' }}">{{$playlist->status == 1 ? 'Active' : 'Deactive' }}</span>
                        </td>
                        <td>{{date('d-m-Y', strtotime($playlist->created_at))}}</td>
                        <td>{{($playlist->updated_at == $playlist->created_at) ? "N/A" : date('d-m-Y', strtotime($playlist->updated_at))}}</td>
                        <td>
                            <a href="javascript:void(0)" title="View" class="btn btn-sm btn-dark data-show" data-id="{{$playlist->id}}"><i class="fa fa-eye"></i></a>
                            <a href="{{route('video.playlist.edit',$playlist->id)}}" title="Edit" class="btn btn-sm btn-info"><i class="fa fa-edit"></i></a>
                            <a href="{{route('video.playlist.delete',$playlist->id)}}" title="Delete" class="btn btn-sm btn-danger"><i class="fa fa-trash"></i></a>
                            <a href="{{route('video.playlist.status',$playlist->id)}}" title="Change Status" class="btn btn-sm {{$playlist->status == 1 ? 'btn-warning' : 'btn-success'}}"><i class="fa {{$playlist->status == 1 ? 'fa-close' : 'fa-check'}}"></i></a>
                        </td>
                    </tr>
                @endforeach
...
<script>
    $(document).ready(function() {
        $('.data-show').on('click', function() {
            let id = $(this).data('id');
            let _url = ("{{ route('video.playlist.show', ['video_id']) }}");
